...
app.get('/linkDevices', function(req, res) {
	...
	if (senAct.io == "i") {
		...
		if (req.query.hasOwnProperty("period")) {
			var period = parseInt(decodeURIComponent(req.query.period));
			...
			var moistureLevel = parseInt(decodeURIComponent(req.query.moistureLevel));
				...
				db.collection('actuations').aggregate({$sort: {date: 1}}, {$group: {_id: "$linkedActuator.id", value: {$last: "$value"}, date: {$last: "$date"}}}, function(err, result) {
					...
					for (var i=0 ; i<linkedDevices.length ; i++) {
						if (linkedDevices[i].hasOwnProperty('linkedSensor')) {
							...
							//if actuator is a water pump
							if (linkedDevices[i].senAct.id == "2") {
								var alreadyActuated = false;

								for (var j=0 ; j<lastActuations.length ; j++) {
									if (linkedDevices[i].id == lastActuations[j]._id) {
										if (lastActuations[j].date >= startPeriod && lastActuations[j].date <= endPeriod) {
